...
(async () => {
  ...
  try {
    ...
    await run('pig theme cycles all characters and preserves moves on small screens',async page=>{
      await page.setViewportSize({width:320,height:568});await page.locator('.cell').nth(0).click();const before=await state(page);
      await page.locator('#btnCharacter').click();await page.locator('#btnCharacter').click();assert.match(await page.title(),/^Pigdoku/);
      assert.equal(await page.locator('#brandPrefix').innerText(),'Pig');assert.equal(await page.locator('[data-tool="dino"]').innerText(),'Pig');
      assert.equal(await page.locator('.dino use').first().getAttribute('href'),'#pig');assert.match(await page.locator('.cell').nth(0).getAttribute('aria-label'),/pig/);
    ...
    await run('pig completion shares Pigdoku and pig emoji',async page=>{
      await page.locator('#btnCharacter').click();await page.locator('#btnCharacter').click();
      await page.evaluate(()=>Object.defineProperty(navigator,'share',{configurable:true,value:async data=>{window.shared=data.text;}}));
      await page.locator('.cell').nth(6*7+p.sol[6]).click();await tick(page,1500);
      assert.equal(await page.locator('.dino use[href="#pig-happy"]').count(),7);
      await page.locator('#btnShare').click();assert.match(await page.evaluate(()=>window.shared),/^Pigdoku/);assert.equal((await page.evaluate(()=>window.shared)).split('🐷').length-1,7);
    },{'game.normal':almostSolved(p)});
    await run('pig sounds decode and theme survives offline reload',async(page,context)=>{
      await page.locator('#btnCharacter').click();await page.locator('#btnCharacter').click();await page.evaluate(()=>navigator.serviceWorker.ready);await page.reload();await context.setOffline(true);await page.reload();assert.match(await page.title(),/^Pigdoku/);
