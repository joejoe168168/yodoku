...
(async () => {
  ...
  const cards = skins.map(skin => `<section><h2>${skin === 'yo' ? 'Yo · Yodoku' : skin === 'ko' ? 'Ko · Kodoku' : skin === 'pig' ? 'Pip · Pigdoku' : 'Snoo · Sludoko'}</h2><div class="poses">${['normal','happy','oops'].map(face => `<figure><img src="${skin}-${face}.png" alt="${skin} ${face}" width="160" height="160"><figcaption>${face} · <a href="${skin}-${face}.svg">SVG</a> · <a href="${skin}-${face}.png">PNG</a></figcaption></figure>`).join('')}</div><div class="poses">${['hello','celebrate'].map(action => `<figure><picture><source media="(prefers-reduced-motion: reduce)" srcset="${skin}-${action === 'hello' ? 'normal' : 'happy'}.png"><img src="${skin}-${action}.gif" alt="${skin} ${action} animation" width="192" height="192"></picture><figcaption><a href="${skin}-${action}.gif">${action} GIF</a></figcaption></figure>`).join('')}</div><div class="sounds">${Object.keys(sounds).map(event => `<label>${event}<audio controls preload="none" src="sounds/${skin}-${event}.wav"></audio></label>`).join('')}</div></section>`).join('');
