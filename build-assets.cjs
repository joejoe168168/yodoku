...
const skins = ['yo', 'ko', 'pig', 'sloth'];
const background = skin => ({ yo: '#FAF8EF', ko: '#F0EEF6', pig: '#FFF0F4', sloth: '#F2EFDF' })[skin];
...
function makeWav(notes, duration, skin) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const [frequency, start, length, volume = .3] of notes) {
      ...
      const f = frequency * (skin === 'ko' ? .75 : skin === 'pig' ? 1.125 : skin === 'sloth' ? .625 : 1);
...
(async () => {
  for (const skin of skins) {
    for (const face of ['', '-happy', '-oops', '-bow', '-explorer', '-cozy']) {
