...
function makeWav(notes, duration, skin) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const [frequency, start, length, volume = .3] of notes) {
      ...
      const f = frequency * (skin === 'ko' ? .75 : 1);
      const phase = 2 * Math.PI * f * u;
...
(async () => {
  for (const skin of ['yo', 'ko']) {
    ...
    const icon = svg(skin, 'translate(5 3) scale(.9)', skin === 'ko' ? '#F0EEF6' : '#FAF8EF');
    ...
    for (const action of ['hello', 'celebrate']) {
      ...
      for (let frame = 0; frame < 20; frame++) {
        ...
        const source = svg(skin + (action === 'celebrate' ? '-happy' : ''), `translate(5 ${5 + y}) translate(45 45) rotate(${angle}) translate(-45 -45) scale(.9)`, skin === 'ko' ? '#F0EEF6' : '#FAF8EF');
  ...
  console.log(`Created ${files.length + 12} asset files; ${(total/1024).toFixed(0)} KiB total, including the optional gallery and GIF exports.`);
